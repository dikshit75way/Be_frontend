    ...
    <nav className="w-full bg-white border-b border-blue-100 p-4">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          <h4 className="hover:text-indigo-700 cursor-pointer" onClick={()=> router.push("/")}>Events</h4>
          {isAuthenticated ? (
      ...
      {open && (
        <div className="md:hidden mt-4 flex flex-col gap-4 bg-white shadow-lg p-4 rounded-md">
          <h4 className="hover:text-indigo-700  cursor-pointer" onClick={()=> router.push("/booking")}>Bookings</h4>
          <h4 className="hover:text-indigo-700 cursor-pointer" onClick={()=> router.push("/")}>Events</h4>
          {isAuthenticated ? (
